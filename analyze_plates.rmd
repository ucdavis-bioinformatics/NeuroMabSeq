...
idx = smartcounts$LCs.Reported == 0 | smartcounts$HCs.Reported == 0
...
### This code block isn't done and was part of some old filtering approach.
# Get rid of the test plate:
# sc = smartcounts[smartcounts$plate != 'TRIMMER0004-1_P4', ]

# #f = sc[sc$LCs.Reported > 0 & f$HCs.Reported > 0,]
...
```{r DuplicateHeavyChain, }
hca = annotated[annotated$Chain == 'HC', ]
dhc = hca$ASV[duplicated(hca$ASV)]
df = data.frame(ASV=hca$ASV[duplicated(hca$ASV)])

df$DuplicatedIn = sapply(df$ASV, function(x){ paste(hca$Sample_Name[hca$ASV == x], collapse = ', ')})
df$Count = sapply(strsplit(df$DuplicatedIn, ', '), length)
