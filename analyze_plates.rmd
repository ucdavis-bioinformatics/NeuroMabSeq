...
plateMetaData = read.table("../NeuroMabSeq/SampleSheet.txt", header=T, as.is=T, sep='\t')

smartcounts = data.frame()
annotated = data.frame()
asvcounts = data.frame()
for(plate in dir("../01-Processing/", pattern='^TRIMMER0')){
    cat(paste0(plate,'\n'))
    ...
    #Load SampleStatus. There is one entry per well:
    ...
    # smartcounts has two entries per well, one for H and one for L
...
# Get corrected trimmer_ID (MabID)
smartcounts$CorrectedMabID = sampleMetaData$trimmer_id[match(smartcounts$sample_name, sampleMetaData$sample_name)]

# Add sequence data info:
...
stidx = sapply(strsplit(smartcounts$CorrectedMabID, '.', fixed=T), length)
smartcounts$SampleType[stidx == 1] = "oligoclonal"
smartcounts$SampleType[stidx > 1] = "monoclonal"
smartcounts$SampleType[grep("positive", smartcounts$CorrectedMabID, ignore.case=T)] = 'PositiveControl'
smartcounts$SampleType[grep("negative|blank|empty|neagtive", smartcounts$CorrectedMabID, ignore.case=T)] = 'NegativeControl'
...
idx = match(rownames(tbann), smartcounts$sample_name)

smartcounts$LC.K.Called = 0
smartcounts$LC.L.Called = 0
smartcounts$HC.H.Called = 0
smartcounts$HC.G.Called = 0
if('K' %in% colnames(tbann)){smartcounts$LC.K.Called[idx] = tbann[,'K']}
if('L' %in% colnames(tbann)){smartcounts$LC.L.Called[idx] = tbann[,'L']}
if('H' %in% colnames(tbann)){smartcounts$HC.H.Called[idx] = tbann[,'H']}
if('G' %in% colnames(tbann)){smartcounts$HC.G.Called[idx] = tbann[,'G']}
...
Within each of the above date ranges, report a count of:
    ...
    2) how many of these yielded only one heavy and one light chain sequence
...
# Set up index for yield category
# YieldCategory = rep("NA", nrow(ndup))
# YieldCategory[ndup$LCs.Reported == 0 & ndup$HCs.Reported == 0] = 'LC=0 and HC=0' 
# YieldCategory[ndup$LCs.Reported == 1 & ndup$HCs.Reported == 1] = 'LC=1 and HC=1' 
# YieldCategory[ndup$LCs.Reported == 0 & ndup$HCs.Reported == 1] = 'LC=0 and HC=1' 
# YieldCategory[ndup$LCs.Reported == 1 & ndup$HCs.Reported == 0] = 'LC=1 and HC=0' 
# YieldCategory[ndup$LCs.Reported == 0 & ndup$HCs.Reported > 1] = 'LC=0 and HC>1' 
# YieldCategory[ndup$LCs.Reported > 1 & ndup$HCs.Reported == 0] = 'LC>1 and HC=0' 
# YieldCategory[(ndup$LCs.Reported > 0 & ndup$HCs.Reported > 0) & (ndup$LCs.Reported > 1 | ndup$HCs.Reported > 1)] = 'LC>1 or HC>1 (both > 0)'

YieldCategory = rep("NA", nrow(ndup))
YieldCategory[ndup$LC.K.Called == 0 & ndup$HC.H.Called == 0] = 'LC=0 and HC=0' 
YieldCategory[ndup$LC.K.Called == 1 & ndup$HC.H.Called == 1] = 'LC=1 and HC=1' 
YieldCategory[ndup$LC.K.Called == 0 & ndup$HC.H.Called == 1] = 'LC=0 and HC=1' 
YieldCategory[ndup$LC.K.Called == 1 & ndup$HC.H.Called == 0] = 'LC=1 and HC=0' 
YieldCategory[ndup$LC.K.Called == 0 & ndup$HC.H.Called > 1] = 'LC=0 and HC>1' 
YieldCategory[ndup$LC.K.Called > 1 & ndup$HC.H.Called == 0] = 'LC>1 and HC=0' 
YieldCategory[(ndup$LC.K.Called > 0 & ndup$HC.H.Called > 0) & (ndup$LC.K.Called > 1 | ndup$HC.H.Called > 1)] = 'LC>1 or HC>1 (both > 0)'
...
Total number of **monoclonal**, **oligoclonal**, and other types of samples sequenced per funding year.

```{r t001.SamplesPerFunding, echo=FALSE, fig.height=8, fig.width=15, fig.align="center"}
tb = table(ndup$SampleType, yidx)[c("monoclonal","oligoclonal","PositiveControl","NegativeControl"),]
kable(tb) %>%
  kable_styling(bootstrap_options = c("hover", "striped"), full_width = F, 
        position="left", fixed_thead=T) %>%
  row_spec(0, angle = 0)
...
Note that these categories were generated using the number of sequences identifid in each sample where [ANARCI](http://opig.stats.ox.ac.uk/webapps/newsabdab/sabpred/anarci/) could categorize the resulting sequence and predict domains.

```{r t002.SamplesPerFunding, echo=FALSE, fig.height=8, fig.width=15, fig.align="center"}
tb = table(YieldCategory, ndup$SampleType)[c("LC=0 and HC=0","LC=0 and HC=1", "LC=1 and HC=0", "LC=0 and HC>1", "LC>1 and HC=0", "LC=1 and HC=1", "LC>1 or HC>1 (both > 0)"), c("monoclonal","oligoclonal","PositiveControl","NegativeControl")]
...
tb = table(YieldCategory[yidx=='Year2'], ndup$SampleType[yidx=='Year2'])[c("LC=0 and HC=0","LC=0 and HC=1", "LC=1 and HC=0", "LC=0 and HC>1", "LC>1 and HC=0", "LC=1 and HC=1", "LC>1 or HC>1 (both > 0)"), c("monoclonal","oligoclonal","PositiveControl","NegativeControl")]
...
idx = (smartcounts$LCs.Reported == 0 | smartcounts$HCs.Reported == 0 ) & smartcounts$SampleType != 'NegativeControl'
